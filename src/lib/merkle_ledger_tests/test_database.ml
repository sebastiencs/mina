  ...
  ( module struct
    ...
    module Make (Test : Test_intf) = struct
      ...
      let get_random_account () =
        ...
        account_from_rust account

      let rust_get_or_create_account db account =
        let account_id = Account.identifier account in
        let account_id = account_id_to_rust account_id in
        let account = account_to_rust account in
        ignore @@ Rust.database_get_or_create_account db account_id account
      ...
      let%test "merkle_path" =
        Test.with_instance (fun mdb ->
            ...
            (* let empty = Hash.hash_account Account.empty in *)
            (* Printf.eprintf "empty_account:%s\n%!" *)
            (*   (Snark_params.Tick.Field.to_string empty) ; *)
            (* let depth = MT.depth mdb in *)
            (* Printf.eprintf "DEPTH=%d\n%!" depth ; *)
            (* let root = MT.merkle_root mdb in *)
            (* Printf.eprintf "root_hash:%s\n%!" *)
            (*   (Snark_params.Tick.Field.to_string root) ; *)

            (* let empty = Account.empty in *)
            (* let empty = { empty with token_symbol = "seb" } in *)

            (* let empty_with_seb = Hash.hash_account empty in *)
            (* Printf.eprintf "hash_account_with_seb:%s\n%!" *)
            (*   (Snark_params.Tick.Field.to_string empty_with_seb) ; *)

            (* let _loc = create_new_account_exn mdb empty in *)
            (* let root = MT.merkle_root mdb in *)
            ...
            let account1 = get_random_account () in
            let account2 = get_random_account () in
            let account3 = get_random_account () in
            let account4 = get_random_account () in

            let hash1 = Hash.hash_account account1 in
            let hash2 = Hash.hash_account account2 in
            let hash3 = Hash.hash_account account3 in
            let hash4 = Hash.hash_account account4 in
            Printf.eprintf "account1:%s\n%!"
              (Snark_params.Tick.Field.to_string hash1) ;
            Printf.eprintf "account2:%s\n%!"
              (Snark_params.Tick.Field.to_string hash2) ;
            Printf.eprintf "account3:%s\n%!"
              (Snark_params.Tick.Field.to_string hash3) ;
            ...
            let ocaml_path =
              List.map ocaml_path ~f:(fun p ->
                  match p with
                  | `Left h ->
                      "Left:" ^ Snark_params.Tick.Field.to_string h
                  | `Right h ->
                      "Right:" ^ Snark_params.Tick.Field.to_string h )
            in
            let ocaml_path = String.concat ~sep:",\n" ocaml_path in
            Printf.eprintf "\nOCAML_PATH=%s\n%!" ocaml_path ;
            ...
            let addr = MT.Addr.of_string "010" in
            let hash = MT.get_inner_hash_at_addr_exn mdb addr in
            Printf.eprintf "HASH_INNER=%s\n%!"
