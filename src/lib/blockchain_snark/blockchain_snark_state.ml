...
module Statement = struct
  ...
  let get_bin_prot_helpers () =
    ( Protocol_state.Value.Stable.Latest.bin_size_t
    , Protocol_state.Value.Stable.Latest.bin_writer_t
    , Protocol_state.Value.Stable.Latest.bin_reader_t
    , Protocol_state.Value.Stable.Latest.sexp_of_t )
...
module Make (T : sig
  ...
  let tag, cache_handle, p, Pickles.Provers.[ step ] =
    Pickles.compile () ~cache:Cache_dir.cache ~public_input:(Input typ)
      ~get_bin_prot_helpers:Statement.get_bin_prot_helpers
