...
module T = struct
  ...
  let update_pending_coinbase_collection ~depth pending_coinbase_collection
    ...
    | `Update_one stack1 ->
        (* Core.Printf.eprintf !"stack1=%{sexp: Pending_coinbase.Stack_versioned.t}\n%!" stack1; *)
  ...
  module Resources = struct
    ...
    let init ~constraint_constants
      ...
      let coinbase, singles =
        init_coinbase_and_fee_transfers ~constraint_constants cw_seq
          ...
          ~supercharge_coinbase
      in
      (* Core.Printf.eprintf !"coinbase_=%{sexp: Coinbase_fee_transfer.t Staged_ledger_diff.At_most_two.t}\n%!" coinbase; *)
      let fee_transfers =
    ...
    let discard_user_command t =
      ...
      in
      (* Core.Printf.eprintf "discard_user_command %d\n%!" (Sequence.length t.commands_rev); *)
      match Sequence.next t.commands_rev with
      | None ->
          ( (* Core.Printf.eprintf "discard_user_command empty\n%!"; *)
  ...
  let rec check_constraints_and_update ~constraint_constants
    ...
    if Resources.slots_occupied resources = 0 then
      ((* Core.Printf.eprintf "slots_occupied=0\n%!"; *)
       resources, log)
    else if Resources.work_constraint_satisfied resources then
      if
        (* Core.Printf.eprintf "work_constraint_satisfied\n%!"; *)

        (*There's enough work. Check if they satisfy other constraints*)
        Resources.budget_sufficient resources
      then
        if
          (* Core.Printf.eprintf "budget_sufficient\n%!"; *)
          Resources.space_constraint_satisfied resources
        then
          ( (* Core.Printf.eprintf "space_constraint_satisfied\n%!"; *)
            resources
          , log )
          ...
          (* Core.Printf.eprintf "worked_more\n%!"; *)
        ...
        (* Core.Printf.eprintf "budget_sufficient NOT\n%!"; *)
      ...
      (* Core.Printf.eprintf "work_constraint_satisfied NOT\n%!"; *)
    ...
    O1trace.sync_thread "create_staged_ledger_diff_one_prediff" (fun () ->
        let init_resources =
          Resources.init ~constraint_constants ts_seq cw_seq slot_job_count
            ...
            ~supercharge_coinbase
        in
        (* Core.Printf.eprintf !"init_resources %{sexp: Coinbase_fee_transfer.t Staged_ledger_diff.At_most_two.t}\n%!" init_resources.coinbase; *)
        (* Core.Printf.eprintf !"init_resources %{sexp: Resources.t}\n%!" init_resources; *)
        let log =
        ...
        in
        (* Core.Printf.eprintf !"init_resources AFTER %{sexp: Coinbase_fee_transfer.t Staged_ledger_diff.At_most_two.t}\n%!" (fst res).coinbase; *)
        res )
    ...
    | Some y ->
        ...
        let res1, res2 =
          if Sequence.is_empty res.commands_rev then
            ...
            (* let r, _ = res in *)
            (* Core.Printf.eprintf !"AAA %{sexp: Coinbase_fee_transfer.t Staged_ledger_diff.At_most_two.t}\n%!" r.coinbase; *)
            (res, None)
          else
            (* Core.Printf.eprintf "BBB %d\n%!" (Resources.available_space res); *)
        ...
        let coinbase_added =
          ...
          + Option.value_map
              ...
              res2 ~default:0
        in
        Core.Printf.eprintf "coinbase_added=%d\n%!" coinbase_added ;
  ...
  let create_diff
    ...
    O1trace.sync_thread "create_staged_ledger_diff" (fun () ->
        ...
        Core.Printf.eprintf
          !"partitions=%{sexp: Scan_state.Space_partition.t}\n%!"
          partitions ;
        Core.Printf.eprintf "work_to_do=%d\n%!" (List.length work_to_do) ;
        ...
        in
        (* Core.Printf.eprintf "completed_works_seq_len=%d\n%!" (Sequence.length completed_works_seq); *)
        (* Core.Printf.eprintf "valid_on_this_ledger_len=%d\n%!" (Sequence.length valid_on_this_ledger); *)
        (* Core.Printf.eprintf "invalid_on_this_ledger_len=%d\n%!" (List.length invalid_on_this_ledger); *)
        (* Core.Printf.eprintf !"invalid_on_this_ledger_len=%{sexp: (User_command.Valid.t * Error.t) list}\n%!" invalid_on_this_ledger; *)
        let diff, log =
        ...
        in
        (* Core.Printf.eprintf !"generated diff=%{sexp: Staged_ledger_diff.With_valid_signatures_and_proofs.pre_diff_with_at_most_two_coinbase * Staged_ledger_diff.With_valid_signatures_and_proofs.pre_diff_with_at_most_one_coinbase option}\n%!" diff; *)
        let%map diff =
...
let%test_module "staged ledger tests" =
  ( module struct
    ...
    let create_and_apply_with_state_body_hash
      ...
      in
      (* Core.Printf.eprintf !"diff created %{sexp: Staged_ledger_diff.With_valid_signatures_and_proofs.t}\n%!" diff; *)
    ...
    *)
    ...
    let proofs stmts : Ledger_proof.t One_or_two.t =
      ...
      One_or_two.map stmts ~f:(fun statement ->
          (* Core.Printf.eprintf !"sok_digest=%{sexp: Sok_message.Digest.t}\n%!" sok_digest; *)
    ...
    let stmt_to_work_one_prover (stmts : Transaction_snark_work.Statement.t) :
        Transaction_snark_work.Checked.t option =
      Core.Printf.eprintf !"stmt_to_work_one_prover\n%!" ;
      Core.Printf.eprintf !"fee=%{sexp: Fee.t}\n%!" work_fee ;
      (* Core.Printf.eprintf !"stmts=%{sexp: Transaction_snark_work.Statement.t}\n%!" stmts; *)
      (* Core.Printf.eprintf !"proofs=%{sexp: Ledger_proof.t One_or_two.t}\n%!" (proofs stmts); *)
      Core.Printf.eprintf !"prover=%{sexp: Account.key}\n%!" snark_worker_pk ;
    ...
    *)
    ...
    let test_simple :
     ...
     fun account_ids_to_check cmds cmd_iters sl ?(expected_proof_count = None)
      ...
      let%map total_ledger_proofs =
        iter_cmds_acc cmds cmd_iters 0
          (fun cmds_left count_opt cmds_this_iter proof_count ->
            Core.Printf.eprintf "######## Start new batch %d ########\n%!"
              !niters ;
            Core.Printf.eprintf "nto_applied=%d\n%!"
            ...
            let cmds_applied_this_iter =
              List.length @@ Staged_ledger_diff.commands diff
            in
            Core.Printf.eprintf "cmds_applied_this_iter=%d\n%!"
              cmds_applied_this_iter ;
            let cb = coinbase_count diff in
            Core.Printf.eprintf "cb=%d\n%!" cb ;
            if !niters = 11 then
              Core.Printf.eprintf !"diff=%{sexp: Staged_ledger_diff.t}\n%!" diff ;
            ( match provers with
            | `One_prover ->
                assert (cb = 1)
            | `Many_provers ->
            ...
            (* Core.Printf.eprintf !"scan_state=%{sexp: Scan_state.t}\n%!" !sl.scan_state; *)
            (* Core.Printf.eprintf "scan_state=%s\n%!" (Scan_state.snark_job_list_compact_yojson !sl.scan_state); *)
            Core.Printf.eprintf
              "######## Batch %d done: %d applied ########\n\n%!" !niters
              cmds_applied_this_iter ;
      ...
      (* let list = Ledger.to_list ledger in *)
      (* Core.Printf.eprintf !"list=%{sexp: Account.t list}\n%!" list; *)
      let scan_state = Sl.scan_state !sl in
      let jobs = Scan_state.base_jobs_on_latest_tree scan_state in
      Core.Printf.eprintf !"jobs_len=%d\n%!" (List.length jobs) ;
      Core.Printf.eprintf
        !"jobs=%{sexp: Scan_state.Transaction_with_witness.t list}\n%!"
        jobs ;
    ...
    let gen_below_capacity ?(extra_blocks = false) () =
      let open Quickcheck.Generator.Let_syntax in
      let _a = extra_blocks in
      let%bind ledger_init_state = Ledger.gen_initial_ledger_state in

      Core.Printf.eprintf
        !"ledger_init_state=%{sexp: Ledger.init_state}\n%!"
        ledger_init_state ;
    ...
    (*         ( *)
    (*           Core.Printf.eprintf !"init_state=\n%{sexp:Ledger.init_state}\n%!" ledger_init_state; *)
    (*           Core.Printf.eprintf !"cmds_len=%d\n%!" (List.length cmds); *)
    (*           Core.Printf.eprintf !"iters_len=%d\n%!" (List.length iters); *)
    (*           Core.Printf.eprintf !"iters_len=%{sexp:int option list}\n%!" iters; *)
    (*           Core.Printf.eprintf !"accounts=%{sexp:Account_id.t list}\n%!" (init_pks ledger_init_state); *)
    (*           Core.Printf.eprintf !"cmds=\n%{sexp:Mina_base.User_command.Valid.t list}\n%!" (List.take cmds 5); *)
    ...
    (*   Quickcheck.test ~trials:1 gen_zkapps_at_capacity *)
    (*     ~f:(fun (ledger, zkapps, iters) -> *)

    (*       (\* Control.(Tag.equal Proof (tag (Account_update.authorization p))) *\) *)
    (*       let _dummy = Control.(dummy_of_tag Proof) in *)
    ...
    (*   Quickcheck.test (gen_below_capacity ()) ~trials:1 *)
    (*     ~f:(fun (ledger_init_state, cmds, iters) -> *)
    (*       async_with_ledgers ledger_init_state (fun sl test_mask -> *)
    (*           test_simple *)
    (*             (init_pks ledger_init_state) *)
    ...
    let%test_unit "Be able to include random number of commands (One prover)" =
      Core.Printf.eprintf
        !"snark_worker_pk=%{sexp: Account.key}\n%!"
        snark_worker_pk ;
      Core.Printf.eprintf
        !"coinbase_receiver=%{sexp: Account.key}\n%!"
        coinbase_receiver ;
      Quickcheck.test (gen_below_capacity ()) ~trials:1
        ~f:(fun (ledger_init_state, cmds, iters) ->
          async_with_ledgers ledger_init_state (fun sl test_mask ->
    ...
    (*             | Ok (diff, _invalid_txns) -> *)
    (*                ( *)
    (*                  Core.Printf.eprintf !"diff_here=%{sexp: Staged_ledger_diff.With_valid_signatures_and_proofs.t}\n%!" diff; *)
    (*               assert ( *)
    (*                 List.is_empty *)
    (*                   (Staged_ledger_diff.With_valid_signatures_and_proofs *)
    (*                    .commands diff ) )) *)
